...
(function ($) {
	...
	function countdownTimer() { 
		...
		$('#countdown_timer').countdown({
			...
			/*d1 = triangles.countdown_date;
			d = new Date(d);
			d.setMonth(d.getMonth() + 1);*/
			until: new Date(settings.countdownDate), 
			timezone: 1,
			format: 'DHMS',
			layout: 
				'<ul>' +
				'{d<}<li class="days"><em>{dn}</em>  DNI</li>{d>}' +
				'{h<}<li class="hours"><em>{hn}</em>  GODZIN</li>{h>}' +
				'{m<}<li class="minutes"><em>{mn}</em>  MINUT</li>{m>}' +
				'{s<}<li class="seconds"><em>{sn}</em>  SEKUND</li>{s>}' +
				'</ul>',
